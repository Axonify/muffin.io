...
ViewHelpers = require 'libs/ViewHelpers'
...
class App
  ...
  initialize: ->
    ...
    # # Set a global success handler
    # $(document).on 'ajaxSuccess', @globalSuccessHandler
    # 
  ...
  # globalSuccessHandler: (event, xhr, settings) =>
  #   # No message shown for GET requests
  #   return if settings.type is 'GET'
  #   
  #   # Check if the model object was passed to $.ajax()
  #   model = settings.sender
  #   return unless model
  #   
  #   modelClass = model.className
  #   handlers = str.messages?[modelClass]?[settings.type]
  #   @flash.success handlers?.success, model.toJSON()
  # 
  ...
  #   modelClass = model.className
  #   handlers = str.messages?[modelClass]?[settings.type]
  #   try errCode = JSON.parse(xhr.responseText).errCode
  #   if errCode and handlers?[errCode]
  #     @flash.error handlers[errCode], model.toJSON()
  #   else
  #     @flash.error handlers?.error, model.toJSON()
